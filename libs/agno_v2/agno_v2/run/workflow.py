...
from agno_v2.run.agent import RunEvent, RunOutput, run_output_event_from_dict
from agno_v2.run.base import BaseRunOutputEvent, RunStatus
from agno_v2.run.team import TeamRunEvent, TeamRunOutput, team_run_output_event_from_dict
...
if TYPE_CHECKING:
    from agno_v2.workflow.types import StepOutput, WorkflowMetrics
else:
    StepOutput = Any
    WorkflowMetrics = Any
...
class WorkflowRunOutput:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "WorkflowRunOutput":
        ...
        from agno_v2.workflow.step import StepOutput

        workflow_metrics_dict = data.pop("metrics", {})
        workflow_metrics = None
        if workflow_metrics_dict:
            from agno_v2.workflow.workflow import WorkflowMetrics
        ...
        for event in events_data or []:
            if "agent_id" in event:
                ...
                from agno_v2.run.agent import run_output_event_from_dict

                event = run_output_event_from_dict(event)
            elif "team_id" in event:
                # Team event from team step
                from agno_v2.run.team import team_run_output_event_from_dict
