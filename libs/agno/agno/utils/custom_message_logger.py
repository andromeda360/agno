...
def log_message(
    ...
    """Log the message to the console with optional system message truncation.

    This is a custom feature for Andromeda360 that allows truncating system messages
    for better readability in logs.

    Args:
        message (Message): The message to log
        system_message_truncate_length (int, optional): If provided, truncates system message
            content to this length. Controlled by AGNO_SYSTEM_MESSAGE_LOG_TRUNCATE_LENGTH env var.
        metrics (bool): Whether to log the metrics. Defaults to True.
        level (str, optional): The level to log the message at. One of debug, info, warning, or error.
            Defaults to debug.
    """
    _logger = log_debug
    if level == "info":
    ...
    if message.content:
        ...
        # Custom feature: Truncate system messages for readability
